...
test_that("build_values_map reproduces the legacy target names", {
  ...
  expect_equal(nrow(vm), 12 * length(test_dbs) * 2)
  expect_false(any(duplicated(vm$full_name)))
  raw <- vm[vm$preprocess == "none", ]
  expect_equal(
    raw$full_name,
    legacy_full_name(raw$method, raw$db, raw$min_bootstrap, raw$vote_algorithm, raw$nb_voting)
  )
  expect_true(all(c(
    "dada2__DB_A___0.5",
    "sintax__DB_B_v2.1_Fungi_cut___0.4",
    "blastn__DB_B_v2.1_Fungi_cut___0.5...rel_majority...100"
  ) %in% raw$full_name))
  expect_equal(nrow(build_values_map(dbs = db_list)), 12 * (length(db_list) + length(itsx_db_list)))
})
...
test_that("the ITSx input runs on itsx_dbs only, the Fungi databases of config.R", {
  ...
  expect_equal(sum(vm$preprocess == "itsx"), 12)
  expect_true(all(itsx_db_list %in% db_list))
  expect_setequal(itsx_db_list, benchmark_dbs$db[benchmark_dbs$simplification %in% c("Fungi", "Fungi+cut")])
})
...
test_that("ITSx rows prefix the method and read the ITSx phyloseq", {
  ...
  expect_true("itsx_blastn__DB_A___0.5...rel_majority...100" %in% itsx$full_name)
  expect_equal(unique(itsx$input_pq), "d_asv_itsx_for_assignation")
  expect_equal(unique(raw$input_pq), "d_asv_for_assignation")
  expect_equal(nrow(build_values_map(dbs = test_dbs, preprocess = "none")), nrow(raw))
})
...
test_that("every database gets its own reference file, in mini and full mode", {
  for (mini in c(FALSE, TRUE)) {
    ...
    expect_equal(dplyr::n_distinct(vm$db_path[vm$method == "sintax"]), length(db_list))
    expect_equal(dplyr::n_distinct(vm$db_path[vm$method == "dada2"]), length(db_list))
    expect_true(all(mapply(\(d, p) endsWith(p, paste0("/", prefix, d, ".fasta")), vm$db, vm$db_path)))
    cv <- build_cv_values_map(dbs = db_list, mini_db = mini)
    expect_true(all(mapply(\(d, p) endsWith(p, paste0("/", prefix, d, ".fasta")), cv$db, cv$db_path)))
  }
})
...
test_that("each reference file has one file target, named without the mini prefix", {
  for (mini in c(FALSE, TRUE)) {
    ...
    expect_setequal(dplyr::distinct(cv, ref_file, db_path)$ref_file, pairs$ref_file)
    expect_setequal(dplyr::distinct(cv, ref_file, db_path)$db_path, pairs$db_path)
  }
  vm_mini <- build_values_map(dbs = test_dbs, mini_db = TRUE)
  expect_true("ref_sintax__DB_B_v2.1_Fungi_cut" %in% vm_mini$ref_file)
  expect_equal(unique(vm_mini$ref_file[vm_mini$method == "dada2" & vm_mini$db == "DB_A"]), "ref_dada2__DB_A")
  expect_false(any(vm_mini$ref_file %in% vm_mini$full_name))
})

test_that("one compute target per method × database × input serves three rows", {
  vm <- build_values_map(dbs = test_dbs, itsx_dbs = test_dbs)
  counts <- table(vm$compute_name)
  expect_equal(length(counts), 4 * length(test_dbs) * 2)
  ...
  per_compute <- dplyr::distinct(vm, compute_name, method, db, preprocess, input_pq, ref_file, controller)
...
test_that("min_cover is set on blastn rows only, from config.R by default", {
  ...
  vm_70 <- build_values_map(dbs = "DB_A", methods = build_methods_grid(min_cover = 70))
...
test_that("CV queries of a _Fungi_cut database come from its _Fungi source", {
  cv <- build_cv_values_map(dbs = c("DB_A", "DB_B_v2.1_Fungi", "DB_B_v2.1_Fungi_cut"))
  cut <- cv[cv$db == "DB_B_v2.1_Fungi_cut", ]
  expect_true(all(cut$query_db == "DB_B_v2.1_Fungi"))
  expect_equal(unique(cut$query_ref_file[cut$method == "sintax"]), "ref_sintax__DB_B_v2.1_Fungi")
  expect_equal(unique(cut$query_db_path[cut$method == "dada2"]),
  ...
  cut_sources <- sub("_cut$", "", benchmark_dbs$db[benchmark_dbs$simplification == "Fungi+cut"])
  expect_true(all(cut_sources %in% db_list))
})

test_that("CV records are named by the sintax_format identifiers of the same database", {
  cv <- build_cv_values_map(dbs = c("DB_A", "DB_B_v2.1_Fungi", "DB_B_v2.1_Fungi_cut"))
  expect_equal(cv$id_db_path, paste0("data/data_raw/refseq/sintax_format/", cv$db, ".fasta"))
  expect_equal(cv$id_ref_file, paste0("ref_sintax__", cv$db))
  expect_equal(cv$query_id_db_path, paste0("data/data_raw/refseq/sintax_format/", cv$query_db, ".fasta"))
  ...
  mini <- build_cv_values_map(dbs = c("DB_A", "DB_B_v2.1_Fungi_cut"), mini_db = TRUE)
  expect_equal(mini$id_db_path, paste0("data/data_raw/refseq/sintax_format/mini_", mini$db, ".fasta"))
})
...
test_that("config: sources, benchmarked databases and special picks are consistent", {
  ...
  expect_true(reference_sources$provider[reference_sources$source == fake_ref_source] == "unite")
  expect_true(is.numeric(blastn_min_cover) && blastn_min_cover > 0 && blastn_min_cover <= 100)
})
